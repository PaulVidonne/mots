...
PRESENTATION GENERALE
   ...
   Objet  le script $(basename $0) version $version est une sur-couche de la commande 'locate'
          ...
          de données 'mlocate.db' générée par la commande 'updatedb'.
   ...
   Mise en garde
          ...
          des répertoires. Reposant sur la base de données 'mlocate.db', et contrairement à la
          commande shell, le script ne déclarera pas les fichiers détruits depuis la génération
...
message="::: Les résultat sont exprimés en date de dernière modification. Recherche de la chaîne '$pattern'"
...
for i in $@ ; do
   if [ $i == "-rm" ] ; then
      ...
      message="::: Les résultat sont exprimés en date de dernier accès. Recherche de la chaîne '$pattern'"
      temps="--time=atime"
   elif [ $i == "-rc" ] ; then
      message="::: Les résultat sont exprimés en date de dernier changement d'attributs. Recherche de la chaîne '$pattern'"
      temps="--time=ctime"
   elif [ $i == "-rb" ] ; then
      message="::: Les résultat sont exprimés en date de création. Recherche de la chaîne '$pattern'"
      temps="--time=birth"
   fi
done
